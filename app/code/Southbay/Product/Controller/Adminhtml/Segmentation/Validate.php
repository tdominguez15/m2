    ...
    public function execute()
    {
        ...
        if ($id) {
            ...
            if (is_null($item)) {
                ...
                $result_data['messages'][] = __('No existe la segmentacion que intenta actualizar');
            }
        } else {
            $collection->addFieldToFilter('code', $code);
            $collection->addFieldToFilter('label', $label);

            if ($collection->getSize() > 0) {
                $result_data['error'] = true;
                $result_data['messages'][] = __('Ya existe otra segmentacion con el mismo  codigo');
            }
        }
